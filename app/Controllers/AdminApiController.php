...
final class AdminApiController
{
    ...
    public function saveProduct(array $params=[]):never
    {
        ...
        try{$pdo=Database::connection();if($id){$sql='UPDATE products SET name=?,slug=?,category=?,description=?,badge=?,image=?,features=?,specifications=?,gallery=?,published=?,featured=?,sort_order=?,show_factory=? WHERE id=?';$values[]=$id;$s=$pdo->prepare($sql);$s->execute($values);if(!$s->rowCount())Response::json(['error'=>'Không tìm thấy sản phẩm.'],404);}else{$s=$pdo->prepare('INSERT INTO products(name,slug,category,description,badge,image,features,specifications,gallery,published,featured,sort_order,show_factory) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)');$s->execute($values);$id=(int)$pdo->lastInsertId();}Response::json(['ok'=>true,'id'=>$id]);}catch(\PDOException $error){Response::json(['error'=>$error->getCode()==='23000'?'Tên đường dẫn đã tồn tại.':'Không thể lưu sản phẩm.'],409);}
    }
    public function deleteProduct(array $params):never{$this->guard();$s=Database::connection()->prepare('DELETE FROM products WHERE id=?');$s->execute([(int)$params['id']]);Response::json(['ok'=>(bool)$s->rowCount()]);}
    ...
    public function deleteLookup(array$params):never{$this->guard();$s=Database::connection()->prepare('DELETE FROM glass_lookup_products WHERE id=?');$s->execute([(int)$params['id']]);Response::json(['ok'=>(bool)$s->rowCount()]);}
